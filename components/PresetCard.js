...
const buildBottlePath = () => {
  const nW = NECK_W, bW = BODY_W, nH = NECK_H, bH = BODY_H
  const cx = CX
...
const PresetCard = () => {
  ...
  const handleScroll = (e) => {
    ...
    if (snapped !== goal) { flash(); setGoal(snapped) }
  }
        ...
        <View style={styles.leftSection}>
          ...
          <View style={{ width: BW, height: BH, position: 'relative' }}>
            ...
            <Svg width={BW} height={BH} style={StyleSheet.absoluteFill} pointerEvents="none">
              ...
              <Rect
                x={CX - BODY_W / 2} y={waterTop}
                width={BODY_W} height={BH - waterTop}
                fill="url(#waterGrad)"
                clipPath="url(#bottleClipFill)"
              />
              <Rect
                x={CX - BODY_W / 2} y={waterTop}
                width={BODY_W} height={1.5}
            ...
            <View style={{
              ...
              <ScrollView
                ref={scrollRef}
                style={{ flex: 1 }}
                contentContainerStyle={{ paddingVertical: (BODY_H - ITEM_HEIGHT) / 2 }}
                  ...
                  return (
                    <View key={val} style={{ height: ITEM_HEIGHT, justifyContent: 'center', alignItems: 'center' }}>
                      <Text style={{
                        ...
                        color: isActive ? '#21C8FF' : 'rgba(33,200,255,0.2)',
            ...
            {/* Bottle outline */}
            <Svg width={BW} height={BH} style={StyleSheet.absoluteFill} pointerEvents="none">
              <Path d={BOTTLE_PATH} fill="none" stroke="rgba(33,200,255,0.35)" strokeWidth="1.5" />
              {[0.33, 0.66].map((f, i) => (
                <Line
                  key={i}
                  x1={CX - BODY_W / 2 + 8} y1={NECK_H + BODY_H * f}
                  x2={CX + BODY_W / 2 - 8} y2={NECK_H + BODY_H * f}
...
const styles = StyleSheet.create({
  ...
  tagText: { color: '#21C8FF', fontSize: 11, fontWeight: '700', letterSpacing: 0.5 },
  sectionLabel: {
    color: '#4A7AB5',
    fontSize: 10,
    fontWeight: '700',
  ...
  fillBarLabel: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 },
  ...
  statNum: { color: '#21C8FF', fontWeight: '800', fontSize: 17, lineHeight: 21 },
  statLabel: { color: '#4A7AB5', fontSize: 10, fontWeight: '600', letterSpacing: 0.4, marginTop: 1 },
  scrollFadeTop: {
    position: 'absolute',
